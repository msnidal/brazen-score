...
class SwinTransformer(nn.Module):
  ...
  """
  def __init__(self, embedding_dim:int, num_heads:int=8, window_dim:int=8, apply_shift:bool=False):
    super().__init__()
    ...
    position_bias = nn.parameter.Parameter(torch.Tensor(size=(num_heads, position_bias_dim, position_bias_dim)))
  ...
  def self_attention(self, windows:torch.Tensor):
    ...
    key_transposed = einops.rearrange(key, "batch num_windows num_heads num_patches embedding -> batch num_windows num_heads embedding num_patches")
    attention_logits = (torch.matmul(query, key_transposed) / math.sqrt(self.dimension)) + self.position_bias # + self.mask
...
class SwinTransformerBlock(nn.Module):
  ...
  def forward(self, embeddings:torch.Tensor):
    if self.apply_shift:
      embeddings = embeddings.roll(self.apply_shift, dims=(1, 2)) # TODO: explore einops.reduce
